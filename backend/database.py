...
class DATABASE:
    ...
    def save_table(self, database_type, values):
        ...
        with self.database_connection:
            self.database_cursor.execute(
                """SELECT 
                    ...
                    WHERE SENDER_ID = ?
                    OR RECEIVER_ID = ?
                    ORDER BY TIME_STAMP DESC;""", (user_id, user_id)
            )

            transaction_history = self.database_cursor.fetchall()
            return transaction_history

masterDB = DATABASE()
masterDB.create_table(database_type=DATABASE.DATABASE_TYPES.USERS)
masterDB.create_table(database_type=DATABASE.DATABASE_TYPES.TRANSACTIONS)
